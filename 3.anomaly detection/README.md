...
학습 단계에서는 아래 그림처럼 정상 데이터만을 통해 오토인코더를 학습한다. 이 단계에서 오토인코더는 정상 패턴을 학습하게 된다.
...
이상 탐지 단계에서는 학습이 완료된 오토인코더에 새로운 이미지를 넣어서 복원값을 구한다.

만약 이상 데이터가 들어오게 된다면 정상 데이터만으로 학습한 오토인코더가 잘 복원하지 못할 것이라고 가정하고 실제값과 복원값의 차이를 통해 anomaly score를 정의하게 된다.
